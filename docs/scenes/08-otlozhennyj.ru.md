# Часть VIII. Отложенный рендеринг
...
|---|---|---|---|
| 0 | `RGBA8_SRGB` | Альбедо · rgb, металличность · a | 4 |
| 1 | `RGB10A2_UNORM` | Нормаль в мировых координатах · rgb, шероховатость · a | 4 |
| 2 | `R32_FLOAT` | Линейная глубина в пространстве вида | 4 |
| — | `D32_FLOAT` | Вложение глубины: только для теста, никогда не читается | 4 |

**12 байт на пиксель** — очень компактно по меркам отложенного рендеринга.

Несколько решений здесь неочевидны и требуют объяснения.

**Альбедо в цели формата `RGBA8_SRGB`.** Восьми бит на канал хватает ровно потому, что формат
sRGB: аппаратура кодирует при записи и декодирует при чтении, а sRGB-кривая как раз уплотняет
градации там, где глаз чувствительнее — в тёмных тонах. Линейное альбедо в тех же восьми битах
дало бы заметные ступеньки в тенях. Оба преобразования бесплатны.

**Нормаль в `RGB10A2_UNORM` — 4 байта вместо 8.** Десять бит на ось достаточно, чтобы ступени на
гладких затенённых поверхностях исчезли; восьми бит (`RGBA8`) для этого не хватает, а
`RGBA16_FLOAT` вдвое дороже и точнее, чем нужно. Оставшиеся два бита уходят под шероховатость —
негусто, но её и хранят обычно в 8 битах, а разницу между четырьмя градациями и 256 на глаз
поймать трудно. Если шероховатость важна, положите её отдельной целью `R8_UNORM`: получится
13 байт вместо 12.

Формат беззнаковый, поэтому нормаль надо перевести в [0, 1] при записи и обратно при чтении:

```metal
// запись
out.normal = float4(normal * 0.5 + 0.5, roughness);
// чтение
const float3 N = normalize(normalSample.xyz * 2.0 - 1.0);
```

**Линейная глубина отдельной целью, а не чтением буфера глубины.** Вот здесь — важное
ограничение обёртки, и его надо назвать прямо.
...
восстановления позиции, для тумана и для SSAO в главе 12.
...
```cpp
constexpr ImageFormat kGBufferAlbedo = ImageFormat::RGBA8_SRGB;
constexpr ImageFormat kGBufferNormal = ImageFormat::RGB10A2_UNORM;
...
// Одно на проход → uniform-блок, слот 1.
struct PassUniforms {
    float4x4 viewProjection;
    float4x4 view;            // нужен для линейной глубины
};

// На вызов → push-константы. 80 байт из гарантированных 128.
//
// Две матрицы 4x4 в push-константах не поместились бы — это ровно 128 байт, и
// на материал места не осталось бы. Разделение по частоте изменения решает это
// само собой: матрицы камеры одинаковы для всего прохода, матрица модели нет.
...
vertex GBufferVertexOut gbuffer_vertex(const device MeshVertex* vertices  [[buffer(0)]],
                                       constant PassUniforms&   pass      [[buffer(1)]],
                                       constant DrawConstants&  constants [[buffer(8)]],
                                       uint                     vid       [[vertex_id]])
{
    MeshVertex v = vertices[vid];
    const float4 world = constants.model * float4(float3(v.position), 1.0);

    GBufferVertexOut out;
    out.clipPosition = pass.viewProjection * world;
    out.worldNormal  = (constants.model * float4(float3(v.normal), 0.0)).xyz;
    out.uv           = float2(v.uv);

    // В правосторонней системе камера смотрит вдоль −Z, поэтому расстояние — это −z.
    out.viewDepth = -(pass.view * world).z;
    return out;
}
                                     ...
                                     bool                    facing    [[front_facing]])
{
    ...
    if (constants.material.z > 0.5) {
        // Текстура в sRGB-формате, цель тоже: аппаратура декодирует при выборке
        // и закодирует при записи. В шейдере всё линейное, преобразований нет.
    ...
    // RGB10A2 беззнаковый: нормаль отображается из [-1, 1] в [0, 1].
    out.normal      = float4(normal * 0.5 + 0.5, constants.material.x);
...
## 8.5. Восстановление позиции без обращения матриц
...
главе 4, **в MSL нет `inverse()` для `float4x4`**, а считать обращение на CPU и передавать 64
...
## 8.6. Проход освещения: полный экран или объёмы
...
проблема из главы 7, только теперь без overdraw.
        ...
        sampler                  smp     [[sampler(0)]])
{
    ...
    // Альбедо уже линейное: цель в формате sRGB, аппаратура декодировала при выборке.
    const float3 albedo    = albedoSample.rgb;
    const float  metallic  = albedoSample.a;
    const float3 N         = normalize(normalSample.xyz * 2.0 - 1.0);   // обратно в [-1, 1]
...
```
запись G-буфера   3.7 млн × 12 Б = 44 МБ
глубина                          = 29.6 МБ
чтение G-буфера в проходе света  = 44 МБ
запись HDR                       = 29.6 МБ
                                   ───────
                                   148 МБ/кадр → 8.9 ГБ/с
```

Вдвое с половиной больше трафика — это **фиксированная плата** за вход, независимо от числа
источников. Обратите внимание, во что обошёлся один правильно подобранный формат: замена нормали
с `RGBA16_FLOAT` на `RGB10A2_UNORM` сняла 29 МБ на кадр, то есть 1.8 ГБ/с, без единого видимого
последствия.
...
больше, чем должны были бы. В главе 14 указано, что нужно добавить в обёртку, чтобы это исправить
— и это самая крупная по отдаче доработка из всего списка для платформы Apple.

---

## 8.8. Чего мы лишились
...
переезжает в постобработку (FXAA, TAA — глава 12), и качество там принципиально ниже.
...
**два** набора шейдеров для одних и тех же материалов. Глава 10.
...
| Ступени на затенении гладких поверхностей | Нормаль упакована в 8 бит; нужен `RGB10A2_UNORM` |
| Нормали «съехали», освещение неверное | Забыт перевод `*0.5+0.5` / `*2-1` для беззнакового формата |
| Тёмные ступени в тенях | Альбедо в `RGBA8_UNORM` вместо `RGBA8_SRGB` |
...
1. Отложенный рендеринг превращает `геометрия × источники` в сумму. Плата — фиксированный трафик
   G-буфера, 12 байт на пиксель в нашей раскладке. **Выбор формата здесь — это прямая экономия
   пропускной способности**: `RGB10A2` вместо `RGBA16F` под нормаль снимает 1.8 ГБ/с.
...
Дальше — [Часть VIII. Тени](09-teni.ru.md): без них сцена остаётся плоской, а объекты — парящими.
