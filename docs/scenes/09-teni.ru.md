...
**Ограничение 1: буфер глубины нельзя сэмплировать.** Как разбиралось в главе 8, изображение с
...
мировых единицах**, а не нелинейную глубину, и это делает подбор смещения (раздел 9.4) намного
понятнее — смещение измеряется в сантиметрах, а не в безразмерных единицах буфера.

---

## 9.2. Карта теней направленного света
...
становятся заметно ступенчатыми. Отсюда каскады (раздел 9.6).
                               ...
                               constant ShadowPassUniforms& pass [[buffer(1)]])
{
    ...
    // тогда и смещение в разделе 9.4 задаётся в сантиметрах, а не в долях буфера.
...
## 9.3. Выборка тени в проходе освещения
...
как не платить за это линейно, — раздел 9.7.

---

## 9.4. Смещение: два дефекта, которые нельзя убрать одновременно
...
Код выборки тени из раздела 9.3 работает без изменений — деление на `w` там уже есть именно для
этого случая.

---

## 9.6. Точечный источник: кубическая карта
...
## 9.7a. Дальность теней — и почему её недооценивают
...
Двести источников света в главе 8 и двести теней — принципиально разные по стоимости вещи.
Отсутствие тени у мелкой лампы почти незаметно; отсутствие тени от солнца заметно всегда.

---

## 9.8a. Диагностика: выгружайте саму карту
...
## 9.8b. Пять ошибок, которые эта глава стоила на практике

Всё описанное выше было реализовано на San Miguel, и по дороге вылезли пять ошибок. Ни одна не
опечатка; каждая — из тех, где симптом уводит в сторону от причины. Они здесь именно поэтому.

Пятая — не про тени вовсе, а про то, **как их проверяли**, и она обошлась дороже остальных
четырёх вместе взятых. Начну с неё.

### 0. Проверка шла не по тому пути, по которому идёт картинка

Тени отлаживались выгрузкой кадра в PNG: отдельный путь, который рендерит сцену в собственное
изображение и читает его обратно. Удобно, воспроизводимо, не требует смотреть в окно.

И на этих выгрузках тени были. А в окне их не было.

Причина оказалась ровно одна строка. Построение списка загораживателей и запись проходов каскадов
были **двумя отдельными вызовами**, и каждый путь рендеринга делал их сам:

```cpp
// путь выгрузки:
buildShadowLists();                                   // ← есть
for (c) recordShadowCascade(cmd, c);
...
В окне проходы каскадов исправно выполнялись — с **пустым** списком. Карта очищалась значением
«ничего не мешает», сравнение в шейдере проходило везде, и сцена выглядела просто освещённой.
Ни ошибки валидации, ни предупреждения: пустой список — совершенно законное состояние.

Дальше сработала ловушка похуже. Симптом («тени есть на картинках, но не на экране») естественно
объясняется чем-то, что отличает окно от оффскрина: свопчейн, MSAA, наложение интерфейса. Именно
туда и уходит время — на проверку ImGui, порядка проходов, многовыборочного пути. Всё это
работало.

**Вывод, который стоит унести из главы наравне с математикой теней:** если проверочный путь не
исполняет тот же код, что и путь на экран, он не проверяет ничего. Не «проверяет хуже» — **ничего**,
потому что расхождение между ними и есть самая вероятная ошибка.

Лечится структурой, а не внимательностью:

```cpp
// Одна точка входа. Оба пути вызывают её и физически не могут разойтись.
auto renderShadows = [&](const std::shared_ptr<CommandBuffer>& cmd) {
    if (shadowsEnabled) buildShadowLists();
    else if (shadowMapsInitialised) return;
    for (uint32_t c = 0; c < kCascadeCount; ++c) recordShadowCascade(cmd, c);
    shadowMapsInitialised = true;
};
```

Правило общее: **то, что должно совпадать, должно быть одной функцией, а не двумя вызовами в
правильном порядке.** Порядок сохраняется ровно до следующего рефакторинга.

И проверяемая величина: пустой список загораживателей — это состояние, о котором стоит сообщать.
Счётчик `shadowDraws` в интерфейсе показывает ноль, и ноль на сцене с деревом виден сразу — если
на него смотреть.
...
`constantBias`, хотя они были ни при чём. Нашлось только выгрузкой карты (раздел 9.8a) — и то не
...
Тот же приём, что для пакетов инстансов в главе 6, и по той же причине.
...
| **Тени видны в отладочной выгрузке, но не в окне** | Пути рендеринга разошлись — раздел 9.8b.0 |
| Счётчик загораживателей показывает 0 | Список не построен: тот же раздел |
| **Вся сцена равномерно потемнела**, теней нет | Знак в положении глаза источника — раздел 9.8b |
...
6. **Проверка стоимости.** Измерьте время кадра с тенями и без. Проход теней на 4 каскада — это
   4 полные растеризации сцены; на San Miguel это будет заметно, и в главе 13 мы это оптимизируем
...
6a. **Ноль загораживателей — тоже результат, и его надо видеть.** Выведите счётчик в интерфейс:
   пустой список выглядит на экране в точности как отсутствие теней по любой другой причине.
...
9. **Выгружайте карту теней в файл.** Это первый шаг отладки, а не последний: три из пяти ошибок
   раздела 9.8b иначе не находились.
10. **Но проверяйте по тому же коду, который рисует в окно.** Отдельный проверочный путь, не
    разделяющий кода с основным, доказывает только собственную работоспособность. Общая функция
    вместо двух вызовов в правильном порядке — единственная надёжная защита.

Дальше — [Часть IX. Прозрачность](10-prozrachnost.ru.md): листва Sponza, стекло и то, почему
