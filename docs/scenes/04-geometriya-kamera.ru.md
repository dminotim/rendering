...
## 4.1. Индексный буфер и кеш вершин
...
13 разница будет заметна, и стоит про неё знать.
...
памяти, и на модели в миллионы вершин выигрыш измеримый. Мы вернёмся к этому в главе 13.
...
## 4.3. Точность глубины: почему Sponza мерцает
...
`LessOrEqual` — для reverse-Z нужен `GreaterOrEqual`. Это укусит в главе 10, на прозрачности.
...
дойдём до затухания света (глава 7) — интенсивность падает как обратный квадрат расстояния, и
...
главе 6 мы так и сделаем.

---

## 4.5. Камера, управляемая мышью
...
11; пока просто знайте, что для камеры, управляемой человеком, это несущественно, а для
автоматического вращения — очень даже.

---

## 4.5a. Развлечение: пол в клетку и производные

Модель нужно на что-то поставить, а класть под неё текстуру пока нечем — глава про текстуры
впереди. Сделаем пол **процедурно**: один большой четырёхугольник и шейдер, который сам решает,
какого цвета пиксель.

По дороге разберём `dfdx`/`dfdy` — пару функций, без которых не обходится ни один серьёзный
шейдер, и которые прямо следуют из того, что растеризация идёт квадами 2×2 (глава 1).

### Наивная версия и её беда

```metal
fragment float4 floor_naive(VertexOut in [[stage_in]])
{
    // fract даёт дробную часть; floor от неё — 0 или 1 в шахматном порядке.
    const float2 cell = floor(in.worldPosition.xz);
    const float checker = fmod(cell.x + cell.y, 2.0);
    return float4(float3(mix(0.20, 0.55, checker)), 1.0);
}
```

Вблизи — идеальная шахматка. Вдали — каша из мерцающих точек, которая ползёт при движении
камеры. Причина ровно та же, что у муара на текстуре без мипов (о нём в главе 5): один пиксель
накрывает несколько клеток, а шейдер отвечает за **точку**, а не за площадь. Какая клетка попала
под центр пикселя — случайность, и она меняется от кадра к кадру.

### Версия, которая знает, насколько она мелкая

Аппаратура умеет сказать шейдеру, насколько быстро величина меняется между соседними пикселями.
Это и есть производные: `dfdx(v)` — разность со соседом справа, `dfdy(v)` — с соседом снизу.
Возможны они потому, что квад 2×2 считается одновременно, и соседи физически доступны.

```metal
fragment float4 floor_antialiased(VertexOut in [[stage_in]])
{
    const float2 p = in.worldPosition.xz;

    // Насколько мировых единиц приходится на один пиксель экрана в этой точке.
    // Вблизи — сотые доли, у горизонта — десятки.
    const float2 width = abs(dfdx(p)) + abs(dfdy(p));

    // Треугольная волна периода 2: 0 → 1 → 0. Аналог шахматки, но непрерывный,
    // а значит его можно осреднить аналитически.
    const float2 wave = abs(fract(p * 0.5) * 2.0 - 1.0);

    // Осреднение по ширине пикселя. smoothstep растягивает переход ровно на
    // столько, сколько пиксель накрывает — не больше и не меньше.
    const float2 smoothWave = smoothstep(0.5 - width, 0.5 + width, wave);

    // Комбинируем оси в шахматный порядок. Ближе к горизонту width велика,
    // smoothWave стремится к 0.5, и клетки честно сливаются в ровный серый —
    // а не в мерцающий мусор.
    const float checker = smoothWave.x * smoothWave.y +
                          (1.0 - smoothWave.x) * (1.0 - smoothWave.y);

    return float4(float3(mix(0.20, 0.55, checker)), 1.0);
}
```

Что здесь по-настоящему важно, помимо клеток:

**`dfdx`/`dfdy` есть только во фрагментном шейдере.** В вершинном соседей нет — некому вычитать.

**Их нельзя вызывать в расходящейся ветке.** Если половина квада ушла в `if`, а половина в
`else`, соседние значения не определены, и результат — мусор. Считайте производную **до**
ветвления и пользуйтесь готовым числом внутри. Это же правило распространяется на выборку из
текстуры с автоматическим мипом: она внутри вызывает те же производные.

**Приём «сгладить по ширине пикселя» универсален.** Любой резкий процедурный узор — полосы,
сетка, точки, контуры — сглаживается так же: посчитать, сколько единиц узора приходится на
пиксель, и растянуть переход на эту величину. В главе 10 мы применим ровно эту логику к порогу
альфа-маски.

### Заодно: каркас без отдельной геометрии

Раз уж есть производные, вот приём, который экономит целый режим отрисовки. Рёбра треугольника
можно нарисовать, не рисуя линий: достаточно передать из вершинного шейдера барицентрическую
координату.

```metal
// Вершинный шейдер выдаёт (1,0,0), (0,1,0), (0,0,1) для трёх вершин
// треугольника — по vertex_id % 3 для неиндексированной отрисовки.
fragment float4 wireframe_fragment(VertexOut in [[stage_in]])
{
    const float3 bary = in.barycentric;

    // Расстояние до ближайшего ребра, в пикселях. Деление на производную
    // переводит из «единиц барицентрики» в экранные пиксели, и линия
    // получается одинаковой толщины независимо от размера треугольника.
    const float3 width = fwidth(bary);
    const float3 edge = smoothstep(float3(0.0), width * 1.5, bary);
    const float  line = 1.0 - min(min(edge.x, edge.y), edge.z);

    return float4(mix(float3(0.1), float3(1.0, 0.8, 0.2), line), 1.0);
}
```

`fwidth(v)` — это `abs(dfdx(v)) + abs(dfdy(v))`, та же величина, что выше, только одним вызовом.

Полезно не только как украшение: каркас поверх модели мгновенно показывает плотность сетки, и на
сценах архива из главы 13 по нему сразу видно, где треугольники мельче пикселя — то самое место,
где растеризатор тратит три четверти работы впустую.

---

## 4.6. Отсечение задних граней
...
ровно те случаи, где это нужно. К этому вернёмся в главе 10.
...
Дальше — [Часть IV. Материалы и текстуры](05-tekstury-materialy.ru.md): загрузчик изображений,
